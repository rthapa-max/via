...
export const FIXTURE_TIMEZONE = "Asia/Kathmandu";
...
/** date_label + time are Nepal Time (Asia/Kathmandu, UTC+5:45). */
export function kickoffMsFromFixture({ dateLabel, time }) {
  ...
  const wallClockUtcMs = Date.UTC(date.year, date.month, date.day, clock.hour, clock.minute);
  return wallClockUtcMs - NEPAL_UTC_OFFSET_MS;
}
